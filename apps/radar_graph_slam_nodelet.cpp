...
// nodolet初始化时被调用，是nodelet生命周期回调函数
  virtual void onInit() {
    ...
    if (enable_preintegration) {
      imu_integrator.reset(new ImuIntegrator());
    }
...
private:
  ...
  void cloud_callback(const nav_msgs::OdometryConstPtr& odom_msg, const sensor_msgs::PointCloud2::ConstPtr& cloud_msg) {
   ...
   // Publish TF between /map and /base_link END
    ...
    thisKeyframeTime = cloud_msg->header.stamp;
  ...
  void imu_callback(const sensor_msgs::ImuConstPtr& imu_msg) {
    if (enable_preintegration) {
      imu_integrator->add_imu_message(imu_msg);
    }
  }
  ...
  geometry_msgs::Transform preIntegrationTransform() {
    ...
    const auto& keyframe = keyframe_queue.front();
    Eigen::Isometry3d integrated_pose = imu_integrator->integrate(lastKeyframeTime, thisKeyframeTime, velocity);
    return matrix2transform(ros::Time::now(), integrated_pose.matrix(), "map", "base_link").transform;
  }
...
private:
  ...
  ros::Time thisKeyframeTime;
  ros::Time lastKeyframeTime;
